...
export default function StoryItem({ item, index }: StoryItemProps) {
  ...
  const accentColor = 
    item.type === "education" ? "text-blue-400 border-blue-500/30 bg-blue-500/10" :
    item.type === "personal" ? "text-green-400 border-green-500/30 bg-green-500/10" :
    "text-purple-400 border-purple-500/30 bg-purple-500/10"; 

  return (
    // LAYOUT: flex-row-reverse puts the card on the Left side for Even items
    <div className={`relative flex flex-col md:flex-row items-center ${isEven ? "md:flex-row-reverse" : ""}`}>
      
      {/* 1. SPACER (Keeps the card centered relative to the timeline) */}
      <div className="hidden md:block w-1/2" />

      {/* 2. THE NODE (Center Icon) */}
      ...
      {/* Mobile: pl-20 (to clear the line). Desktop: padding depends on side. */}
        ...
        <FadeUp delay={index * 0.1} className="w-full">
          <div className="relative group">
            ...
            <div className="p-6 rounded-2xl bg-neutral-900/60 border border-white/5 hover:border-white/10 hover:bg-neutral-900 transition-all duration-300 hover:-translate-y-1 shadow-xl backdrop-blur-sm text-left">
              ...
              <p className="text-neutral-300 font-bold mb-4">{item.organization}</p>
              
              <p className="text-neutral-400 text-m leading-relaxed mb-6">
                {item.description}
              </p>

              {/* Responsibilities - Always Left Aligned */}
              {item.responsibilities.length > 0 && (
                 <ul className="mb-6 space-y-2 text-m text-neutral-500 text-left">
              ...
              <div className="flex flex-wrap gap-2 justify-start">
                {item.skills.map((skill) => (
                  <span key={skill} className={`text-[10px] px-2 py-1 rounded border font-mono font-bold uppercase ${accentColor}`}>
                    {skill}
                  </span>
                ))}
              </div>
